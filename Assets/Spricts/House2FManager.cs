...
public class House2FManager : MonoBehaviour
{
    ...
    IEnumerator GirlComment()
    {
        ...
        TextController.Instance.DisplayText("朝になったようだ。家を出よう。");
        m_sound.PlayOneShot(m_sound.clip);
        yield return new WaitForSeconds(3f);
        yield return StartCoroutine("StartFadeIn");
        yield return new WaitForSeconds(3f);
        TextController.Instance.DisplayText("雨が降っている。");
        yield return new WaitForSeconds(3f);
        TextController.Instance.DisplayText("朝だというのに暗い、、、");
        yield return new WaitForSeconds(3f);
        TextController.Instance.DisplayText("懐中電灯を持ってきて良かった。");
        yield return new WaitForSeconds(3f);
        TextController.Instance.DisplayText("目的地は灯台だ、先を急ごう");
        yield return new WaitForSeconds(3f);
