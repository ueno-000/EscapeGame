...
public class Manager3F : MonoBehaviour
{
    ...
    IEnumerator GirlComment()
    {
        ...
        TextController.Instance.DisplayText("灯台が見えてきた",false);
        ...
        TextController.Instance.DisplayText("きっと助けを呼べるものがあるはず", false);
        yield return new WaitUntil(() => Input.GetMouseButtonDown(0));
        yield return null;
        m_soundKey.PlayOneShot(m_soundKey.clip);
        TextController.Instance.DisplayText("ーー灯台の扉を開けたーー", false);
        yield return new WaitUntil(() => Input.GetMouseButtonDown(0));
        yield return null;
